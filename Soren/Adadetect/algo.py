...
def EmpBH(null_statistics, test_statistics, level):
    """Apply BH using empirical p-values from null statistics."""
...
def EmpBH_fast(null_statistics, test_statistics, level):
    """
    Fast implementation from Algorithm 1 of Roquain & Mary.

    Note: this implementation may break when many scores are tied.
    """
    ...
    # np.argsort(-mixed_statistics) gives stats in descending order.
...
def adaptiveEmpBH(null_statistics, test_statistics, level, correction_type, storey_threshold=0.5):
    """Adaptive empirical BH with either Storey or quantile null-proportion correction."""
    pvalues = np.array([compute_pvalue(x, null_statistics) for x in test_statistics])
...
def quantile_estimator(pvalues, k0):
    """k0 is typically around m/2 where m is the number of p-values."""
    m = len(pvalues)
